...
\begin{enumerate}
    ...
    \item $T(t+s)^* = T(t)T(s) \ \forall t ,s \geq 0$. En efecto, 
    \begin{align*}
...
\begin{proof}
    Se deduce inmediatamente del Teorema \ref{ch:semigrupos:section:dual:th:SunDual} y del Lema \ref{ch:semigrupos:section:dual:cor:dualReflex}.
\end{proof}
...
\begin{Corolario}[Espectro de un operador con resolvente compacto]
    ...
    Si un operador $A$ tiene resolvente $\rho(A)$ compacto, entonces $\sigma(A)$ está formado exclusivamente por autovalores de $A$.
\end{Corolario}
...
\begin{Teorema}[Caracterización de la estabilidad exponencial en un espacio de Hilbert]
    ...
    \begin{enumerate}
        ...
        \item $\displaystyle\lim_{\lvert \lambda \lvert \rightarrow \infty}\lVert (i\lambda I - A)^{-1}\rVert_{L(H)} < \infty$
    \end{enumerate}
\end{Teorema}
